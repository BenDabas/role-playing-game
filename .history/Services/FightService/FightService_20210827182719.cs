    ...
    public class FightService : IFightService
    {
        ...
        public async Task<ServiceResponse<FightResultDto>> Fight(FightRequestDto request)
        {
            ...
            try
            {
                ...
                while(!defeated)
                {
                    foreach (var attacker in characters)  // Every attacker fight randomly one opponent.
                    {
                        var opponents = characters.Where(c => c.Id != attacker.Id).ToList();
                        var opponent = opponents[new Random().Next(opponents.Count)]; // Random the opponent.
                        ...
                        if(opponent.HitPoints <= 0)  // If character defeated.
                        {
                            defeated = true;
                            attacker.Victories++;
                            opponent.Defeats++;
        ...
        public async Task<ServiceResponse<List<HighscoreDto>>> GetHighscore()
        {
            var characters = await _context.Characters
                ...
                .ThenBy(c => c.Defeats)  // If the victories is equal then by defeats(the low defeats above).
                .ToListAsync();
        }
    }
}